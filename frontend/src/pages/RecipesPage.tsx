...
export default function RecipesPage() {
  ...
  return (
    <div>
      ...
      {error && <p style={{ color: "#cc3333", marginBottom: 12, fontSize: 14 }}>{error}</p>}
        ...
        recipes.map((r) => (
          <div key={r.id} className="card flex justify-between items-center" style={{ gap: 10 }}>
            <Link to={`/recipe/${r.id}`} style={{ textDecoration: "none", flex: 1, minWidth: 0 }}>
              ...
              {r.description && (
                <div className="text-muted" style={{ marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {r.description}
                </div>
              )}
